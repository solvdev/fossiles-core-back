-- =====================================================================
-- OPK-18: recibir todo EXCEPTO product_id IN (116, 223, 179, 177)
-- =====================================================================
-- Qué hace:
--   1) Abre recepción (warehouse_receipt_closed_at = NULL)
--   2) Unidades de productos 116, 223, 179, 177 → PENDING
...
WHERE UPPER(TRIM(po.code)) = 'OPK-18'
...
-- 2) Productos 116, 223, 179, 177 → PENDING (lo único que debe faltar)
...
WHERE u.shipped_at IS NULL
  ...
  AND u.production_order_item_id IN (
      ...
      WHERE UPPER(TRIM(po.code)) = 'OPK-18'
        AND i.product_id IN (116, 223, 179, 177)
  );
...
WHERE u.shipped_at IS NULL
  ...
  AND u.production_order_item_id IN (
      ...
      JOIN production_order po ON po.id = i.production_order_id
      WHERE UPPER(TRIM(po.code)) = 'OPK-18'
        AND i.product_id NOT IN (116, 223, 179, 177)
  );
...
WHERE i.production_order_id IN (
    SELECT id FROM production_order WHERE UPPER(TRIM(code)) = 'OPK-18'
);
...
SELECT
    i.product_id,
    CASE WHEN i.product_id IN (116, 223, 179, 177) THEN 'PENDIENTE' ELSE 'RECIBIDO' END AS esperado,
    i.quantity AS qty_pedida,
    COALESCE(i.warehouse_received_qty, 0) AS qty_recibida_item,
    COUNT(*) FILTER (WHERE u.receipt_status = 'PENDING') AS unidades_pending,
    COUNT(*) FILTER (WHERE u.receipt_status = 'RECEIVED') AS unidades_received
